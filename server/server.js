...
app.get("/api/users", (req, res) => {
  const sql =
    "SELECT c.id_users, c.surname, c.firstName, c.patronymic, c.contact_info, cc.name_course, c.role, ho.academic_year, ho.status, ho.group_name FROM client c JOIN history_order ho ON c.id_users = ho.id_users JOIN course_category cc ON ho.id_course = cc.id_course;";
...
function client_create(
  ...
  const sql_client =
    "INSERT INTO client (`surname`,`firstName`,`patronymic`,`contact_info`, `role`) VALUES (?, ?, ?, ?, ?)";
  const values_client = [surname, firstName, patronymic, contact_info, role];
...
function id_client_find(
  ...
  const sql_find =
    "SELECT id_users FROM client WHERE surname=? AND firstName=? AND patronymic=? AND contact_info=? AND role=?";
  const filter = [surname, firstName, patronymic, contact_info, role];
